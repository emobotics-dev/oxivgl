...
//!
//! The M5Stack Fire has no touchscreen, so click events will never fire.
//! The button is rendered for visual verification only.
#![no_std]
#![no_main]
...
async fn main(_low_prio_spawner: Spawner) {
    ...
    run_lvgl::<Ex2View, LVGL_BUF_BYTES>(SCREEN_W.into(), SCREEN_H.into(), bufs).await;
}
